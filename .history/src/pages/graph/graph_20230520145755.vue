<template>
  <div class="root">
    ...
    <div
      ...
      class="g6-tooltip"
      :style="`top:${top}px; left:${left}px;opacity: 0.7`"
    >
      {{ tooltip }}
    </div>
  </div>
</template>
<!-- :style="`top:${top}px; left:${left}px;white-space: pre-wrap;max-height: 140px;opacity: 0.7`" -->
<script>
...
export default {
  ...
  data () {  
    return {
      ...
      nodeShapes: [
        ...
        {
          name:  'PLC Smart 200',
          shape: 'custom-image-node',
        },
         {
          name:  '磁性开关',
          shape: 'switch-image-node',
        },
        {
          name:  '工业面相机',
          shape: 'camera-image-node',
        },
        {
          name:  '工业线相机',
          shape: 'lineCamera-image-node',
        },
  ...
  methods: {
    ...
    connectNodes(node) {
      // 设定要连接的id和anchor
      const needConnectNodeList = ['1001', '2001'];
      // 获取放入节点的相关信息
      const nodeId = node.get('id');
      const sourceAnchor = node.getAnchorPoints()[0]; // 假设起始锚点为第一个
      const targetNodes = this.graph.getNodes().filter(n => n.get('id') !== nodeId);  // 得到除第一个节点以外的

      // 遍历目标节点，根据预设的anchorPoints连接关系进行连线
      targetNodes.forEach(targetNode => {
        if(targetNode.getData)
